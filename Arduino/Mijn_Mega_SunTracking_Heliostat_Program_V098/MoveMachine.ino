...
float minAz, float minAlt, float maxAz, float maxAlt){
  ...
  send08(Heliostat);	// Arduino send Heliostat or sunfolger
  //BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
  ...
  if (SunsAltitude>=0 || digitalRead(WindProtectionSwitch)==HIGH || digitalRead(manualModeOnOffPin)==HIGH){//Waits until the sun is above the horizon before moving normally.
    ...
    if (sunTrackerOrHelio == 2){//Machine Acts as Heliostat
      ...
      if (joystickModeOnOff!=1){
        if(FirstIterationAfterArduinoReset!=0){
		  ...
		  //
		  // write to Android
		  if (BTconnected) send21(MachineTargetAlt[machineNumber], MachineTargetAz[machineNumber]);
		  // write to Display
		  // GLCD.SelectFont(System5x7);
		  /* GLCD.CursorTo(9, 4);
		  GLCD.print(MachineTargetAlt[machineNumber], 1);
		  GLCD.CursorTo(16, 4);
		  GLCD.print(MachineTargetAz[machineNumber], 1);
		  */
		  Display_Format(MachineTargetAlt[machineNumber], 7, 1, 7, 4);	// format float to Display on col 7 and row 4
		  Display_Format(MachineTargetAz[machineNumber], 7, 1, 14, 4);	// format float to Display on col 14 and row 2
		  //BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
    ...
    if (joystickModeOnOff!=1){
	  ...
	  //
	  // write to Android
	  if (BTconnected) send20(MachinesNewAltitude, MachinesNewAzimuth);
	  // write to Display
	  GLCD.SelectFont(System5x7);
	  Display_Format(MachinesNewAltitude, 6, 1, 8, 3);	// format float to Display on col 8 and row 3
	  Display_Format(MachinesNewAzimuth, 7, 1, 14, 3);	// format float to Display on col 14 and row 3
	  //BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
    }     
