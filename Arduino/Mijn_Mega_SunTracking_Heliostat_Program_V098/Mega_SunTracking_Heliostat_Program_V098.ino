...
BB Extra coding bij Bernard Drahmann
BB 20161109 declaration of fields for Bluetooth connection
BB 20161109 BT_code added. 
BB 20161215 extra variables in EEPROM (starting from end EEPROM)
BB 
...
// declaration for BB
//
#include <openGLCD.h>		// for Graphical Display

#define LOG_T_INTERVAL 5000		// 5 seconden
uint32_t syncTimet = 0;			// time of last sync() time

long DATARATE = 19200;  // default data rate for BT Bee
char inChar = 0;
String Arduinoinfo = "";
String Androidinfo = "";
String s = "";
...
boolean BTconnected = false;	// true if Android is connected with the Arduino 
int summertime;

int month, year, day, dayOfWeek;	// replaced from loop
float hour, minute, second;			// replaced from loop
byte secondRTC, minuteRTC, hourRTC, dayOfWeekRTC, dayOfMonthRTC, monthRTC, yearRTC;	// replaced from loop
...
void setup(){
  ...
  for (int i=0; i <= numberOfMachines - 1; i++){
	...
	//
	// SETUP for B
	// SETUP several varaibles
	EEPROM.get(4091, latitude);
	EEPROM.get(4087, longitude);
	EEPROM.get(4083, timezone);
	EEPROM.get(4081, useNorthAsZero);
	EEPROM.get(4061, summertime);

	Serial3.begin(DATARATE);	// start BT connection
	GLCD.Init();				// initieer GLCD
...
{  
  ...
  snprintf(buf, sizeof(buf), "%02d:%02d:%02d", hourRTC, minuteRTC, secondRTC);
  // draw the formatted string in the right corner on the display
  GLCD.SelectFont(Wendy3x5);
  GLCD.DrawString(buf, gTextfmt_right, gTextfmt_top);
  // Read and write Android and Arduino info
  Serial.print("*****************BTconnect = "); Serial.println(BTconnected);
  if (BTconnected) SendBT();		// write to Android if Android is connected
  ReadBT();		// read Android info
  ...
  if ((digitalRead(manualModeOnOffPin)!=HIGH) && (joystickModeOnOff!=1)){    
    ...
    if ( (now + updateEvery*1000) < updateTime){
	  ...
	  Serial.println("*********************** update aangesproken");
    }
  ...
  if ((now>=updateTime)||(iterationsAfterReset == 0)){
    ...
    if ((joystickModeOnOff!=1)){
	  ...
	  //
	  // write to Android
	  send18(SunsAltitude, SunsAzimuth);
	  // write to Display
	  GLCD.SelectFont(System5x7);
	  Display_Format(SunsAltitude, 7, 1, 7, 2);	// format float to Display on col 7 and row 2
	  Display_Format(SunsAzimuth, 7, 1, 14, 2);	// format float to Display on col 14 and row 2
	  //BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
  ...
  for (int i=0; i <= numberOfMachines - 1; i++){
    ...
    if (joystickModeOnOff!=1){   
	  ...
	  //
	  // write to Android
	  if (BTconnected) send19(machineNumber);
	  // write to Display
	  GLCD.SelectFont(System5x7);
	  GLCD.CursorTo(7, 3);
	  GLCD.print(machineNumber);
	  //BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB
  ...
  if (digitalRead(WindProtectionSwitch)==HIGH){
    delay(1000); //Serial.println("Wind Protection Mode Enabled"); 
    iterationsAfterReset=0;
  }  
